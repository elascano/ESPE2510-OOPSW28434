...
const csvHeader = "ID;NAME;COLOR;AGE;IS_MOLTING\n";
const csvRows = chickens
  .map(c => `${c.id};${c.name};${c.color};${c.age};${c.isMolting}`)
  .join("\n");
fs.writeFileSync("chickens_history.csv", csvHeader + csvRows, "utf-8");
console.log("Datos guardados correctamente en 'chickens_data.csv'");

let txtContent = "CHICKEN INFORMATION\n";
txtContent += "-------------------\n\n";
for (const c of chickens) {
  txtContent += `ID: ${c.id}\n`;
  txtContent += `NAME: ${c.name}\n`;
  txtContent += `COLOR: ${c.color}\n`;
  txtContent += `AGE: ${c.age}\n`;
  txtContent += `IS MOLTING: ${c.isMolting}\n`;
  txtContent += "-------------------\n";
}
fs.writeFileSync("chickens_history.txt", txtContent, "utf-8");
console.log("Datos guardados correctamente en 'chickens_data.txt'");
