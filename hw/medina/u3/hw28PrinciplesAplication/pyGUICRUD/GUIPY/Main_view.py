...
class AppInterface:
    ...
    def _setup_ui(self):
        # --- Form Area ---
        form_frame = ttk.LabelFrame(self.root, text="Product Form", padding=20)
    ...
    def on_save(self):
        try:
            # 1. Creamos el objeto Product desde los inputs
            new_prod = self._create_product_from_form()
            
            # 2. Calculamos impuestos usando la clase lógica
            new_prod.total_with_tax = TaxCalculator.calculate_tax_for_product(new_prod)

            # 3. Guardamos el objeto usando la clase DB
            if self.db.insert_product(new_prod):
    ...
    def on_update(self):
        try:
            prod = self._create_product_from_form()
            # Recalcular impuesto al actualizar
            prod.total_with_tax = TaxCalculator.calculate_tax_for_product(prod)
            
            if self.db.update_product(prod):
    ...
    def on_calc_total(self):
        """Calcula el total de todos los productos cargados."""
        all_products = self.db.get_all_products() # Retorna lista de Objetos
    ...
    def _create_product_from_form(self):
        ...
        return Product(p_id, desc, price, stock)

    def _fill_form(self, prod):
        """Helper para llenar inputs desde un objeto Product."""
        self.clear_form()
        self.ent_id.insert(0, prod.id)
        self.ent_desc.insert(0, prod.description)
        self.ent_price.insert(0, prod.base_price)
        self.ent_stock.insert(0, prod.stock)
    ...
    def on_select_item(self, event):
        ...
        if sel:
            ...
            # Creamos un objeto temporal para reusar _fill_form
            temp_prod = Product(str(vals[0]), vals[1], vals[2], vals[3]) 
            self._fill_form(temp_prod)
