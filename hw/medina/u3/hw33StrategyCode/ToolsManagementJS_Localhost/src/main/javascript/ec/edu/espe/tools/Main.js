  ...
  static async main() {
    ...
    app.get("/api/tools/:id", async (req, res) => {
      try {
        ...
        if (!t) return res.status(404).json({ ok:false, message:"No encontrado" });
    ...
    app.post("/api/tools", async (req, res) => {
      try {
        ...
        if (!id || !name || !Number.isFinite(price)) return res.status(400).json({ ok:false, message:"Datos inválidos" });
        const ok = await controller.createSculpture(id, name, price, materials);
        if (!ok) return res.status(409).json({ ok:false, message:"No se pudo crear" });
        res.json({ ok:true });
      } catch (e) {
        res.status(500).json({ ok:false, message:e.message });
      }
    ...
    app.put("/api/tools/:id", async (req, res) => {
      try {
        ...
        if (!ok) return res.status(404).json({ ok:false, message:"No se pudo actualizar" });
        res.json({ ok:true });
      } catch (e) {
        res.status(500).json({ ok:false, message:e.message });
      }
    ...
    app.delete("/api/tools/:id", async (req, res) => {
      try {
        ...
        if (!ok) return res.status(404).json({ ok:false, message:"No se pudo eliminar" });
        res.json({ ok:true });
      } catch (e) {
        res.status(500).json({ ok:false, message:e.message });
      }
