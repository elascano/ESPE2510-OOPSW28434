package ec.edu.espe.strategy.utils;

import com.mongodb.client.MongoClient;
import com.mongodb.client.MongoClients;
import com.mongodb.client.MongoDatabase;

/**
 *
 * @author Arelis Samantha Bonilla Cruz, Student, @ESPE
 */
public class MongoConnection {

    private static MongoConnection instance;
    private MongoClient mongoClient;
    private MongoDatabase database;
    private static final String CONNECTION_STRING = "mongodb+srv://Arelis:Arelis2006@cluster0.qdn4zsf.mongodb.net/";
    private static final String DATABASE_NAME = "ParkingDB";

    private MongoConnection() {
        try {
            mongoClient = MongoClients.create(CONNECTION_STRING);
            database = mongoClient.getDatabase(DATABASE_NAME);
            System.out.println("MongoDB connection successful");
        } catch (Exception e) {
            System.err.println("Error connecting MongoDB: " + e.getMessage());
        }
    }

    public static MongoConnection getInstance() {
        if(instance == null) instance = new MongoConnection();
        return instance;
    }

    public MongoDatabase getDatabase() { return database; }
}