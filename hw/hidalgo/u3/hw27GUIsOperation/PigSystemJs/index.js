...
// Support automated command lists via CLI:
//  node index.js --commands=2,0
// or
//  node index.js --script=commands.txt
...
function questionAsync(query) {
    // If commands were provided, return next command instead of waiting for input
    if (commands && cmdIndex < commands.length) {
        const response = commands[cmdIndex++];
        // print prompt + response so output looks like interactive session
