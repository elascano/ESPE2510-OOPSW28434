...
from csv_manager import save_csv, load_csv, display_csv_files
...
def main():
    ...
    while True:
        ...
        if option == "1":
            ...
            print("\nSave data to CSV")
            display_csv_files()
            filename = input("Enter filename to save (or press Enter for 'chicken_farm.csv'): ").strip()
            if not filename:
                filename = "chicken_farm.csv"
            save_csv(coops, filename)
        
        elif option == "8":
            print("\nLoad data from CSV")
            csv_files = display_csv_files()
            if csv_files:
                filename = input("Enter filename to load (or press Enter for 'chicken_farm.csv'): ").strip()
                if not filename:
                    filename = "chicken_farm.csv"
                load_csv(coops, filename)
