...
async function showMenu(coops) {
    while (true) {
        ...
        console.log("7. Save to CSV");
        console.log("8. Load from CSV");
        console.log("9. Exit");
        
        const option = await question("Choose an option (1-9): ");
        ...
        switch (option) {
            ...
            case "7":
                console.log("\nSave data to CSV");
                CSVManager.displayCSVFiles();
                let saveFilename = await question("Enter filename to save (or press Enter for 'chicken_farm.csv'): ");
                if (!saveFilename.trim()) {
                    saveFilename = "chicken_farm.csv";
                }
                if (!saveFilename.endsWith('.csv')) {
                    saveFilename += '.csv';
                }
                CSVManager.saveToCSV(coops, saveFilename);
                break;
                
            case "8":
                console.log("\nLoad data from CSV");
                const csvFiles = CSVManager.displayCSVFiles();
                if (csvFiles.length > 0) {
                    let loadFilename = await question("Enter filename to load (or press Enter for 'chicken_farm.csv'): ");
                    if (!loadFilename.trim()) {
                        loadFilename = "chicken_farm.csv";
                    }
                    CSVManager.loadFromCSV(coops, loadFilename);
                } else {
                    console.log("No CSV files available to load.");
                }
                break;
            ...
            default:
                console.log("Please choose a valid option (1-9).");
        }
    }
}
