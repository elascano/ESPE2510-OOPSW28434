...
class JSONFileManager:
    ...
    def load_from_file():
        try:
            if not os.path.exists(JSONFileManager.FILE_NAME):
                print("No existing data file found. Starting with empty farm.")
                return []
            ...
            if "chickens" in coop_data and isinstance(coop_data["chickens"], list):
                for chicken_data in coop_data["chickens"]:
                    try:
                        ...
                        print(f"Error creating chicken from data: {str(e)}")
                        continue
    ...
    def display_file_data():
        try:
            if not os.path.exists(JSONFileManager.FILE_NAME):
                print("ℹNo data file found. Please save data first.")
                return
    ...
    def delete_file():
        try:
            if os.path.exists(JSONFileManager.FILE_NAME):
                ...
                print(f"File {JSONFileManager.FILE_NAME} deleted successfully.")
            else:
                print(f"ℹFile {JSONFileManager.FILE_NAME} does not exist.")
        except Exception as e:
