...
from ec.edu.espe.instruments.controller.InstrumentController import InstrumentController
from ec.edu.espe.instruments.utils.CsvPersistence import CsvPersistence
from ec.edu.espe.instruments.utils.JsonPersistence import JsonPersistence

class FrmInstrument(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("InstrumentsManagement")
        ...
        self.controller = InstrumentController(self.persistence)
    ...
    def _build(self):
        ...
        ttk.Label(root, text="InstrumentssManagement", font=("Segoe UI", 18, "bold")).pack(anchor="w")
        ...
        ttk.Button(actions, text="Crete", command=self._create).grid(row=0, column=0, padx=(0, 8), pady=4)
        ttk.Button(actions, text="Update", command=self._update).grid(row=0, column=1, padx=(0, 8), pady=4)
        ttk.Button(actions, text="Delete", command=self._delete).grid(row=0, column=2, padx=(0, 8), pady=4)
        ttk.Button(actions, text="Find", command=self._find).grid(row=0, column=3, padx=(0, 8), pady=4)
        ttk.Button(actions, text="Refresh", command=self._refresh).grid(row=0, column=4, padx=(0, 8), pady=4)
        ttk.Button(actions, text="Clean", command=self._clear).grid(row=0, column=5, pady=4)
        ...
        ttk.Label(header, text="List of Instruments", font=("Segoe UI", 12, "bold")).pack(side=tk.LEFT)
    ...
    def _changeStrategy(self, silent):
        ...
        if key == "csv":
            ...
            try:
                from ec.edu.espe.instruments.utils.MongoPersistence import MongoPersistence
    ...
    def _refresh(self):
        self._clearTable()
        try:
            instruments = self.controller.getAllInstruments()
        except Exception as e:
            self._status(str(e))
            return
        ...
        self._status(f"Total: {len(instruments)}")
    ...
    def _create(self):
        ...
        self._status("Create" if ok else "cant be created")
        self._refresh()

    def _update(self):
        id, name, price, materials = self._parse()
        if not id or not name or price is None:
            self._status("Complete ID, Name y Price")
            return
        ok = self.controller.updateSculpture(id, name, price, materials)
        self._status("Updated" if ok else "Cant be updated")
        self._refresh()

    def _delete(self):
        id = self.idVar.get().strip()
        if not id:
            self._status("Enter the ID")
            return
        ok = self.controller.deleteSculpture(id)
        self._status("Deleted" if ok else "Cant be deleted")
        self._refresh()
