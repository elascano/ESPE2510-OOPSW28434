const Chicken = require('../ChickenFarmSimulatorModel/Chicken');
const { makeChickenPoop } = require('../ChickenFarmSimulatorModel/Poop');
const { layEgg } = require('../ChickenFarmSimulatorModel/Egg');
const fs = require('fs');
...
class ChickenCoopSimulator {
    ...
    saveToTXT() {
        let data = "=== Chicken Farm Report ===\n\n";
        const date = new Date();
        data += `Report generated on: ${date.toLocaleDateString()} ${date.toLocaleTimeString()}\n\n`;
        
        for (const coop of this.coops) {
            data += `=== Coop ${coop.name} (ID: ${coop.id}) ===\n`;
            if (coop.chickens.length === 0) {
                data += "No chickens here!\n";
            } else {
                data += "Chickens:\n";
                for (const chicken of coop.chickens) {
                    data += `  - ID: ${chicken.id}\n    Name: ${chicken.name}\n    Color: ${chicken.color}\n    Age: ${chicken.age}\n    Molting: ${chicken.isMolting ? 'Yes' : 'No'}\n\n`;
                }
            }
            data += "\n";
        }
        
        fs.writeFileSync("chickens_report.txt", data);
    }
    ...
    run() {
        ...
        this.saveToTXT();
        console.log("\nFiles generated:");
        console.log("- chickens_data.csv");
        console.log("- chickens_report.txt");
    }
}
