package ec.edu.espe.contactstbook.view;
import utils.Validator;
import ec.edu.espe.contactstbook.controller.ContactDAO;
...
public class FrmContacts extends javax.swing.JFrame {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        int option = JOptionPane.showConfirmDialog(rootPane, "Are you sure you want to save this contact?", "Save Contact", JOptionPane.YES_NO_OPTION);

        if (option == JOptionPane.YES_OPTION) {
            ContactDAO dao = new ContactDAO();
            boolean saved = dao.save(contact);

            if (saved) {
                JOptionPane.showMessageDialog(rootPane, "Saved successfully:\n" + contact.toString());
                emptyFields();
            } else {
                JOptionPane.showMessageDialog(rootPane, "Error saving to Database", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    ...
    private boolean readValues() {
        ...
        if (!Validator.validateName(firstName)) {
            JOptionPane.showMessageDialog(this, "First Name is invalid (only letters).", "Validation Error", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        if (!Validator.validateName(lastName)) {
            JOptionPane.showMessageDialog(this, "Last Name is invalid (only letters).", "Validation Error", JOptionPane.ERROR_MESSAGE);
            return false;
        }

        LocalDate localDate = dtBirthDate.getDate();
        if (localDate == null) {
            JOptionPane.showMessageDialog(this, "Please select a birth date.", "Validation Error", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        Date birthDate = java.sql.Date.valueOf(localDate);
        int age = Validator.computeAge(localDate);

        String typeOfContact = cmbType.getSelectedItem().toString();

        String sex = Validator.getSelectedSex(radSexMale, radSexFemale);

        ArrayList<String> hobbies = Validator.getSelectedHobbies(lstHobbies);
        if (hobbies.isEmpty()) {
