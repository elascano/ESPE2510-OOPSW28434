...
class NaturalPersonsForm(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("AlertSystem")
        self.configure(bg="#d9dde3")

        title = tk.Label(self, text="NATURAL PERSONS", font=("Segoe UI", 22, "bold"), bg="#d9dde3")
        ...
        tk.Label(self, text="LastName:", bg="#d9dde3").grid(row=2, column=0, sticky="e", padx=20)
        self.last_var = tk.StringVar()
        tk.Entry(self, textvariable=self.last_var, width=30).grid(row=2, column=1, pady=5, sticky="w")

        tk.Label(self, text="Identification:", bg="#d9dde3").grid(row=3, column=0, sticky="e", padx=20)
        self.id_var = tk.StringVar()
        tk.Entry(self, textvariable=self.id_var, width=30).grid(row=3, column=1, pady=5, sticky="w")

        tk.Label(self, text="Nationality:", bg="#d9dde3").grid(row=4, column=0, sticky="e", padx=20)

        countries = [
            "Afganistán", "Argentina", "Brasil", "Canadá", "Chile", "China", "Colombia", "Ecuador",
            "España", "Estados Unidos", "Francia", "Italia", "Japón", "México", "Perú", "Venezuela"
        ]

        self.nationality_var = tk.StringVar()
        nationality_combo = ttk.Combobox(self, textvariable=self.nationality_var,
                                         values=countries, width=27, state="readonly")
        nationality_combo.set("Afganistán")
        nationality_combo.grid(row=4, column=1, pady=5, sticky="w")
        ...
        tk.Label(self, text="Gender:", bg="#d9dde3").grid(row=7, column=0, sticky="e", padx=20, pady=(10, 5))
        self.gender_var = tk.StringVar(value="Male")

        tk.Radiobutton(self, text="Male", variable=self.gender_var, value="Male", bg="#d9dde3").grid(row=7, column=1, sticky="w")
        tk.Radiobutton(self, text="Female", variable=self.gender_var, value="Female", bg="#d9dde3").grid(row=7, column=1, sticky="e")

        tk.Button(self, text="SAVE", width=12, command=self.save).grid(row=9, column=0, columnspan=3, pady=25)
    ...
    def open_calendar(self):
        ...
        def choose():
            ...
            d = datetime.strptime(selected, "%d/%m/%Y")
            meses = [
                "enero", "febrero", "marzo", "abril", "mayo", "junio",
                "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"
            ]
            self.birth_var.set(f"{d.day} de {meses[d.month-1]} de {d.year}")
            top.destroy()

        tk.Button(top, text="OK", command=choose).pack(pady=10)
