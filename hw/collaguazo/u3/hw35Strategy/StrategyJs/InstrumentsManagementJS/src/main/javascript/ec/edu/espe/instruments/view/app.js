class FrmInstrument {
  ...
  async changeStrategy(skipMessage) {
    ...
    this.subtitleEl.textContent = `Estrategia: ${strategy === "mongo" ? "MongoPersistence" : strategy === "csv" ? "CsvPersistence" : "JsonPersistence"}`;
    if (!skipMessage) this.status(`Estrategia cambiada`);
    await this.refresh();
  }
  ...
  async refresh() {
    const instruments = await this.api("GET", "/api/instruments");
    this.renderInstruments(instruments);
    this.status(`Total: ${instruments.length}`);
  }

  async create() {
    const { id, name, price, materials } = this.readForm();
    if (!id || !name || !Number.isFinite(price)) { this.status("Completa ID, Name y Price"); return; }
    const res = await this.api("POST", "/api/instruments", { id, name, price, materials });
    this.status(res.ok ? "Creado" : res.message || "No se pudo crear");
    await this.refresh();
  }
  ...
  async update() {
    const { id, name, price, materials } = this.readForm();
    if (!id || !name || !Number.isFinite(price)) { this.status("Completa ID, Name y Price"); return; }
    const res = await this.api("PUT", `/api/instruments/${encodeURIComponent(id)}`, { name, price, materials });
    this.status(res.ok ? "Actualizado" : res.message || "No se pudo actualizar");
    await this.refresh();
  }

  async delete() {
    const id = this.idInput.value.trim();
    if (!id) { this.status("Ingresa ID"); return; }
    const res = await this.api("DELETE", `/api/instruments/${encodeURIComponent(id)}`);
    this.status(res.ok ? "Eliminado" : res.message || "No se pudo eliminar");
    await this.refresh();
  }
  ...
  async find() {
    const id = this.idInput.value.trim();
    if (!id) { this.status("Ingresa ID"); return; }
    const res = await this.api("GET", `/api/instruments/${encodeURIComponent(id)}`);
    if (!res.ok) { this.status(res.message || "No encontrado"); return; }
    const t = res.instrument;
    this.nameInput.value = t.name;
    this.priceInput.value = t.price;
    this.materialsInput.value = (t.materials || []).join(", ");
...
window.addEventListener("DOMContentLoaded", () => new FrmInstrument());
