...
class Main {
  static async main() {
    ...
    const controller = new InstrumentController(strategy);
    ...
    app.post("/api/strategy", (req, res) => {
      ...
      if (!["mongo","csv","json"].includes(key)) return res.status(400).json({ ok:false, message:"Estrategia inválida" });
      setStrategy(key);
      res.json({ ok:true });
    });

    app.get("/api/instrumentss", async (req, res) => {
      try {
        const instruments = await controller.getAllInstruments();
        res.json(instruments.map(t => ({
          id: t.getId(),
          name: t.getName(),
          price: t.getPrice(),
    ...
    app.post("/api/instruments", async (req, res) => {
      try {
        ...
        if (!ok) return res.status(409).json({ ok:false, message:"It cant be created" });
        res.json({ ok:true });
      } catch (e) {
        res.status(500).json({ ok:false, message:e.message });
      }
    ...
    app.put("/api/instruments/:id", async (req, res) => {
      try {
        ...
        if (!ok) return res.status(404).json({ ok:false, message:"It cant be updated" });
        res.json({ ok:true });
      } catch (e) {
        res.status(500).json({ ok:false, message:e.message });
      }
    ...
    app.delete("/api/instruments/:id", async (req, res) => {
      try {
        ...
        if (!ok) return res.status(404).json({ ok:false, message:"It cant be deleted" });
        res.json({ ok:true });
      } catch (e) {
        res.status(500).json({ ok:false, message:e.message });
      }
    ...
    const server = app.listen(port, () => {
      console.log(`Server inicialize in  http://localhost:${port}`);
    });
